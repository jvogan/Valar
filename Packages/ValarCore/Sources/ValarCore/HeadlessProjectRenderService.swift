...
public actor HeadlessProjectRenderService {
    public typealias ChapterSynthesizer = @Sendable (ModelIdentifier, RenderSynthesisOptions, String) async throws -> AudioChunk
    ...
    @discardableResult
    public func enqueueProjectRender(
    ...
    @discardableResult
    public func enqueueChapterRender(
        project: ProjectRecord,
    ...
    private func render(_ job: RenderJob) async throws {
        guard let chapterID = job.chapterID else {
            throw HeadlessProjectRenderError.missingChapterReference(job.id)
        }
        ...
        guard Self.isRenderableChapter(chapter) else {
            throw HeadlessProjectRenderError.emptyChapterScript(chapter.id)
        }
        guard let bundleLocation = await projectStore.bundleLocation(for: job.projectID) else {
            throw HeadlessProjectRenderError.missingBundleLocation(job.projectID)
        }
    ...
    public static func outputFileName(for chapter: ChapterRecord) -> String {
    ...
    public static func isRenderableChapter(_ chapter: ChapterRecord) -> Bool {
        !chapter.script.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    }

    public static func validatedOutputURL(for outputFileName: String, in exportsDirectory: URL) throws -> URL {
        ...
        guard outputURL.path.hasPrefix(exportsDirectoryPath) else {
            throw HeadlessProjectRenderError.pathTraversal(outputFileName)
        }

        return outputURL
    }

    public static func sanitizeFileStem(_ value: String) -> String {
    ...
    public static func failureReason(from error: any Error) -> String {
        let preferredMessage = ValarPathRedaction.sanitizeMessage(
...
public enum HeadlessProjectRenderError: Error, Sendable {
...
extension HeadlessProjectRenderError: LocalizedError {
