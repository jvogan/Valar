...
public class Qwen3ASRModel: Module {
    ...
    public static func fromDirectory(_ modelDir: URL) async throws -> Qwen3ASRModel {
        // Load config
        ...
        try ensureQwenTokenizerJSONPresent(in: modelDir)
        ...
        for searchDirectory in [modelDir, modelDir.appendingPathComponent("weights", isDirectory: true)]
            where fileManager.fileExists(atPath: searchDirectory.path)
        {
            let files = try fileManager.contentsOfDirectory(at: searchDirectory, includingPropertiesForKeys: nil)
...
func ensureQwenTokenizerJSONPresent(in modelDir: URL) throws {
