...
public enum TTS {
    public static func fromDirectory(_ modelDir: URL) async throws -> SpeechGenerationModel {
        let modelType = try ModelUtils.resolveModelType(modelDirectory: modelDir)
        return try await loadModel(modelDirectory: modelDir, modelType: modelType)
    }
    ...
    public static func loadModel(
        ...
        return try await loadModel(modelRepo: modelRepo, modelType: modelType, hfToken: hfToken, cache: cache)
    }
    ...
    public static func loadModel(
        ...
        let resolvedType = normalizedModelType(modelType) ?? inferModelType(from: modelDirectory.lastPathComponent)
        ...
        switch resolvedType {
        case "voxtral_tts":
            return try VoxtralTTSModel.fromDirectory(modelDirectory)
        case "soprano_tts", "soprano":
            return try await SopranoModel.fromDirectory(modelDirectory)
        case "tada":
            return try await loadTadaPack(from: modelDirectory)
        case "qwen3_tts":
            return try await Qwen3TTSModel.fromDirectory(modelDirectory)
        case "qwen3", "qwen":
            return try await Qwen3Model.fromDirectory(modelDirectory)
        case "vibevoice_streaming", "vibevoice":
            return try await VibeVoiceTTSModel.fromDirectory(modelDirectory)
        default:
    ...
    public static func loadModel(
        ...
        try await loadModel(modelRepo: modelRepo, modelType: modelType, hfToken: nil, cache: cache)
    }
    ...
    static func loadModel(
        ...
        case "voxtral_tts":
            return try await VoxtralTTSModel.fromPretrained(modelRepo, cache: cache)
        case "tada":
            guard let repoID = Repo.ID(rawValue: modelRepo) else {
                throw TTSModelError.invalidRepositoryID(modelRepo)
            }
            let modelDir = try await ModelUtils.installSelfContainedTadaPack(
                repoID: repoID,
                hfToken: hfToken,
                cache: cache
            )
            return try await loadTadaPack(from: modelDir)
        case "vibevoice_streaming", "vibevoice":
            return try await VibeVoiceTTSModel.fromPretrained(modelRepo, hfToken: hfToken)
        default:
            throw TTSModelError.unsupportedModelType(modelType ?? resolvedType)
        }
    }
    ...
    private static func normalizedModelType(_ modelType: String?) -> String? {
        ...
        switch lowercased {
        case "tada_tts", "tada-tts", "mlx_tada", "mlx-tada":
            return "tada"
        case "qwen3_tts_tokenizer_12hz":
            return "qwen3_tts"
        default:
            if lowercased.hasPrefix("qwen3_tts") {
                return "qwen3_tts"
            }
            return lowercased
        }
    ...
    private static func loadTadaPack(from modelDirectory: URL) async throws -> SpeechGenerationModel {
        try ModelUtils.validateTadaPackLayout(modelDirectory: modelDirectory)
        return try await TADATTSModel.fromDirectory(modelDirectory)
    }
}
