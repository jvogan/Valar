...
public class SopranoModel: Module, KVCacheDimensionProvider, SpeechGenerationModel, @unchecked Sendable {
    ...
    public static func fromPretrained(
        ...
        return try await loadFromDirectory(modelDir, versionHint: modelRepo)
    }

    public static func fromDirectory(_ modelDir: URL) async throws -> SopranoModel {
        try await loadFromDirectory(modelDir, versionHint: modelDir.lastPathComponent)
    }
    ...
    private static func loadFromDirectory(
        ...
        let normalizedVersionHint = versionHint.lowercased()
            .replacingOccurrences(of: "_", with: "-")
        let isSoprano11 =
            normalizedVersionHint.contains("soprano-1.1") ||
            normalizedVersionHint.contains("soprano-1-1")
        if !isSoprano11 {
