...
public struct SopranoConfiguration: Codable, Sendable {
    ...
    public init(from decoder: Swift.Decoder) throws {
        ...
        // Audio config — Soprano 1.1 outputs at 24 kHz; fall back to 24000 when the
        // model's config.json omits "sample_rate" (the HuggingFace bf16 checkpoint
        // does not include this field, so the default drives the AudioChunk.sampleRate
        // that flows into the OGG/Opus encoder path).
