...
final class CodePredictorModel: Module {
    ...
    func callAsFunction(
        ...
        if let positionEmbeddings {
            ...
            posEmbeddings = rotaryEmb(inputsEmbeds, positionIds: posIds)
        }
...
final class Qwen3TTSCodePredictor: Module {
    private struct PreparedGenerationPosition {
        let cos: MLXArray
        let sin: MLXArray
        let mask: MLXArray?
    }
    ...
    private let preparedGenerationPositions: [PreparedGenerationPosition]
    ...
    init(config: Qwen3TTSTalkerCodePredictorConfig, talkerHiddenSize: Int) {
        ...
        let predictorModel = CodePredictorModel(config: config, talkerHiddenSize: talkerHiddenSize)
        _model.wrappedValue = predictorModel
        _lmHead.wrappedValue = (0 ..< config.numCodeGroups - 1).map { _ in
            Linear(config.hiddenSize, config.vocabSize, bias: false)
        }
        self.preparedGenerationPositions = Self.makePreparedGenerationPositions(
            rotary: predictorModel.rotaryEmb,
    ...
    func callAsFunction(
        ...
        let preparedPosition = preparedGenerationPositions[safe: generationStep]
        let preparedEmbeddings = preparedPosition.map {
            ($0.cos.asType(embeds.dtype), $0.sin.asType(embeds.dtype))
        }
        let resolvedMask = preparedPosition?.mask.map { $0.asType(embeds.dtype) } ?? mask
        let x = model(
            embeds,
            positionIds: positionIds,
            positionEmbeddings: preparedEmbeddings,
            mask: resolvedMask,
            cache: cache
        )
    ...
    private static func makePreparedGenerationPositions(
        rotary: Qwen3TTSRotaryEmbedding,
        numCodeGroups: Int
    ) -> [PreparedGenerationPosition] {
        guard numCodeGroups > 1 else { return [] }

        var prepared = [PreparedGenerationPosition]()
        prepared.reserveCapacity(numCodeGroups - 1)

        let initialPositionIDs = MLXArray([Int32(0), Int32(1)]).reshaped(1, 2)
        let initialEmbeddings = rotary(
            MLXArray.zeros([1, 2, 1], type: Float.self),
            positionIds: initialPositionIDs
        )
        prepared.append(
            PreparedGenerationPosition(
                cos: initialEmbeddings.0,
                sin: initialEmbeddings.1,
                mask: MultiHeadAttention.createAdditiveCausalMask(2).asType(.float32)
            )
        )

        guard numCodeGroups > 2 else { return prepared }
        for generationStep in 1 ..< (numCodeGroups - 1) {
            let position = Int32(generationStep + 1)
            let positionIDs = MLXArray([position]).reshaped(1, 1)
            let embeddings = rotary(
                MLXArray.zeros([1, 1, 1], type: Float.self),
                positionIds: positionIDs
            )
            prepared.append(
                PreparedGenerationPosition(
