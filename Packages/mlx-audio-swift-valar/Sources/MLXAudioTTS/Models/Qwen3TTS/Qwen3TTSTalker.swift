...
private func applyRotaryPosEmb(
    ...
    let qEmbed = q * cosVal + rotateHalf(q) * sinVal
    let kEmbed = k * cosVal + rotateHalf(k) * sinVal
...
final class Qwen3TTSTalkerModel: Module {
    struct PreparedGenerationPositions {
        let cos: MLXArray
        let sin: MLXArray

        subscript(safe index: Int) -> (MLXArray, MLXArray)? {
            guard index >= 0, index < cos.dim(2) else {
                return nil
            }
            return (
                cos[0..., 0..., index ..< (index + 1), 0...],
                sin[0..., 0..., index ..< (index + 1), 0...]
            )
        }
    }
    ...
    func callAsFunction(
        ...
        let preparedPositionEmbeddings = preparedPositionEmbeddings(from: posEmbeddings)

        let attentionMask: MLXFast.ScaledDotProductAttentionMaskMode = {
            if let mask {
                return .array(mask)
            }
            if seqLen <= 1, cache?.first == nil {
                return .none
            }
            return createAttentionMask(h: inputsEmbeds, cache: cache?.first)
        }()

        var x = inputsEmbeds
        for (i, layer) in layers.enumerated() {
            x = layer(x, positionEmbeddings: preparedPositionEmbeddings, mask: attentionMask, cache: cache?[i])
        }
        return norm(x)
    }
    ...
    func prepareGenerationPositions(
        startOffset: Int,
        count: Int,
        dtype: DType
    ) -> PreparedGenerationPositions? {
        guard count > 0 else {
            return nil
        }

        let positions = MLXArray(Int32(startOffset) ..< Int32(startOffset + count)).reshaped(1, count)
        let positionIDs = stacked([positions, positions, positions], axis: 0)
        let placeholder = MLXArray.zeros([1, count, 1], dtype: dtype)
        let (cos, sin) = rotaryEmb(placeholder, positionIds: positionIDs)
        return PreparedGenerationPositions(
            cos: expandedDimensions(cos, axis: 1),
            sin: expandedDimensions(sin, axis: 1)
        )
    }

    private func preparedPositionEmbeddings(
        from positionEmbeddings: (MLXArray, MLXArray)
    ) -> (MLXArray, MLXArray) {
        let (cos, sin) = positionEmbeddings
        guard cos.ndim == 3, sin.ndim == 3 else {
            return positionEmbeddings
        }
        return (
            expandedDimensions(cos, axis: 1),
            expandedDimensions(sin, axis: 1)
        )
    }
}
...
final class Qwen3TTSTalkerForConditionalGeneration: Module {
    ...
    func prepareGenerationPositions(
        ...
        count: Int,
        dtype: DType
    ) -> Qwen3TTSTalkerModel.PreparedGenerationPositions? {
