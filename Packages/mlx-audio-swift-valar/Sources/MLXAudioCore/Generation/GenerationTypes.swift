...
public struct AudioGenerationInfo: Sendable {
    ...
    public let evalTime: TimeInterval?
    public let tokenMaterializationTime: TimeInterval?
    public let embeddingAssemblyTime: TimeInterval?
    public let talkerForwardTime: TimeInterval?
    public let codePredictorTime: TimeInterval?
    ...
    public init(
        ...
        samplingTime: TimeInterval? = nil,
        evalTime: TimeInterval? = nil,
        tokenMaterializationTime: TimeInterval? = nil,
        embeddingAssemblyTime: TimeInterval? = nil,
        talkerForwardTime: TimeInterval? = nil,
        codePredictorTime: TimeInterval? = nil,
        ...
        self.tokenMaterializationTime = tokenMaterializationTime
        self.embeddingAssemblyTime = embeddingAssemblyTime
        self.talkerForwardTime = talkerForwardTime
        self.codePredictorTime = codePredictorTime
    ...
    public var summary: String {
        ...
        Sampling: \(String(format: "%.3f", samplingTime ?? 0))s
        Eval: \(String(format: "%.3f", evalTime ?? 0))s
        Token Materialization: \(String(format: "%.3f", tokenMaterializationTime ?? 0))s
        Embedding Assembly: \(String(format: "%.3f", embeddingAssemblyTime ?? 0))s
        Talker Forward: \(String(format: "%.3f", talkerForwardTime ?? 0))s
        Code Predictor: \(String(format: "%.3f", codePredictorTime ?? 0))s
...
public enum AudioGeneration: Sendable {
    ...
    case audio(MLXArray)
    /// Raw codec frame, shape [1, numCodeGroups], one per generation step.
    /// Used by Valar's native decoder path to bypass upstream audio decoding.
    case codes(MLXArray)
}
